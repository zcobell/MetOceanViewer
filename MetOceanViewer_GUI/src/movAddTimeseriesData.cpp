...
mov_dialog_addtimeseries::mov_dialog_addtimeseries(QWidget *parent)
    : QDialog(parent), ui(new Ui::mov_dialog_addtimeseries) {
  ui->setupUi(this);
...
//-------------------------------------------//
mov_dialog_addtimeseries::~mov_dialog_addtimeseries() { delete ui; }
//-------------------------------------------//
...
//-------------------------------------------//
void mov_dialog_addtimeseries::set_default_dialog_box_elements(
    int NumRowsInTable) {
  QString ButtonStyle;
...
//-------------------------------------------//
void mov_dialog_addtimeseries::set_dialog_box_elements(
...
void mov_dialog_addtimeseries::on_browse_filebrowse_clicked() {
  ...
  if (TempPath != NULL || (TempPath == NULL && this->CurrentFileName != NULL)) {
    ...
    this->FileReadError = false;
    this->InputFileType =
        movFiletypes::getIntegerFiletype(this->CurrentFileName);
...
void mov_dialog_addtimeseries::on_button_presetColor1_clicked() {
  ...
  this->RandomButtonColor =
      MovColors::styleSheetToColor(ui->button_seriesColor->styleSheet());
  ui->button_seriesColor->update();
}
...
void mov_dialog_addtimeseries::on_combo_variableSelect_currentIndexChanged(
    const QString &arg1) {

...
QString MovSession::getSessionFilename() { return this->sessionFileName; }
...
int MovSession::save() {
  ...
  ierr = MovGeneric::NETCDF_ERR(nc_def_dim(ncid, "ntimeseries",
  ...
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(
      ncid, "timeseries_filename", NC_STRING, 1, dims_1d, &varid_filename));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_colors", NC_STRING,
                                           1, dims_1d, &varid_colors));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_names", NC_STRING,
  ...
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_coldstartdate",
                                           NC_STRING, 1, dims_1d,
                                           &varid_coldstart));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_stationfile",
                                           NC_STRING, 1, dims_1d,
                                           &varid_stationfile));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_xshift", NC_DOUBLE,
                                           1, dims_1d, &varid_xshift));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_yshift", NC_DOUBLE,
                                           1, dims_1d, &varid_yshift));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_units", NC_DOUBLE,
  ...
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_layer", NC_INT, 1,
  ...
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_xlabel", NC_STRING,
                                           1, dims_1d, &varid_xlabel));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_ylabel", NC_STRING,
                                           1, dims_1d, &varid_ylabel));
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_precision", NC_INT,
  ...
  if (ierr != NC_NOERR)
    return 1;
  ierr = MovGeneric::NETCDF_ERR(nc_def_var(ncid, "timeseries_autodate", NC_INT,
  ...
  mydataint[0] = 3;
  ierr =
      MovGeneric::NETCDF_ERR(nc_put_var_int(ncid, varid_precision, mydataint));
  if (ierr != NC_NOERR)
    return 1;
  mydatadouble[0] = this->yMinSpinBox->value();
  ierr =
      MovGeneric::NETCDF_ERR(nc_put_var_double(ncid, varid_ymin, mydatadouble));
  if (ierr != NC_NOERR)
    return 1;
  mydatadouble[0] = this->yMaxSpinBox->value();
  ierr =
      MovGeneric::NETCDF_ERR(nc_put_var_double(ncid, varid_ymax, mydatadouble));
  if (ierr != NC_NOERR)
    return 1;
  ...
  if (this->checkAllData->isChecked())
    ...
    mydataint[0] = 0;
  ierr =
      MovGeneric::NETCDF_ERR(nc_put_var_int(ncid, varid_autodate, mydataint));
  if (ierr != NC_NOERR)
    return 1;
...
int MovSession::open(QString openFilename) {
  ...
  ierr =
      MovGeneric::NETCDF_ERR(nc_open(openFilename.toUtf8(), NC_NOWRITE, &ncid));
  if (ierr != NC_NOERR)
    return 1;

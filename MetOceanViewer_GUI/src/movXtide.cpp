...
//...Constructor
movXtide::movXtide(QWebEngineView *inMap, MovQChartView *inChart, QDateEdit *inStartDateEdit,
...
QString movXtide::getErrorString()
{
...
QString movXtide::getCurrentXTideStation()
{
...
QString movXtide::getLoadedXTideStation()
{
...
int movXtide::getClickedXTideStation()
{
    QString tempString;
    QVariant eval = QVariant();
    this->map->page()->runJavaScript("returnStationID()",[&eval](const QVariant &v){eval = v;});
...
int movXtide::parseXTideResponse(QString xTideResponse)
{
...
int movXtide::getDataBounds(double &min, double &max)
{
    int j;
...
int movXtide::saveXTideData(QString filename, QString format)
{
...
int movXtide::saveXTidePlot(QString filename, QString filter)
{
    if(filter == "PDF (*.pdf)")
    {
